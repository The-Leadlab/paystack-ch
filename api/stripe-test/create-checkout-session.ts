...
import { runCreateCheckoutSession } from "../../lib/stripeBilling.js";
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    ...
    const out = await runCreateCheckoutSession(req.headers.authorization, body, req.headers, true);
    stripeCorsApplyHeaders(req, res);
    res.status(out.status).json(out.json);
  } catch (e) {
    console.error("[api] stripe-test create-checkout-session:", e);
    if (!res.headersSent) {
      try {
        stripeCorsApplyHeaders(req, res);
      } catch {
