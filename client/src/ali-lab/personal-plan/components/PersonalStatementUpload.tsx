  ...
  const onFile = async (file: File | undefined) => {
    ...
    try {
      ...
      if (!preview.rows.length) {
        setErr(preview.issues[0] || t("stmtAiNoRows"));
        return;
      }

      setPhase("filling");
      const filled = await refinePersonalDraftsWithAi(preview.rows.map((r) => ({ ...r, selected: true })));
      ...
      setSuccess(
        t("stmtAiImported")
          .replace("{n}", String(record.rowCount))
          .replace("{income}", formatChfDisplay(record.incomeTotal))
          .replace("{expense}", formatChfDisplay(record.expenseTotal))
      );
      onImported();
    } catch (e) {
