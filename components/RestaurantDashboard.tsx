...
export function RestaurantDashboard() {
  ...
  const handleDocumentData = async (data: FinancialData, fileName: string) => {
    const date = data.date || new Date().toISOString().split('T')[0];
    const amount = data.amountInCHF || data.totalAmount || 0;
    
    // Categorize based on document type and amount
    ...
    if (docType === 'Bank Statement' || docType === 'Bank Deposit') {
      ...
      if (data.lineItems) {
        for (const item of data.lineItems) {
          if (item.type === 'INCOME') {
            await addIncome(item.date || date, 'SALES', item.amount, item.description || fileName);
          } else if (item.type === 'EXPENSE') {
            await addExpense(item.date || date, 'OTHER', item.amount, item.description || fileName);
          }
        }
      }
      ...
      await addExpense(date, category as any, amount, data.notes || fileName);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row">
      ...
      <div className="flex-1 p-4 md:p-8 overflow-y-auto">
        ...
        <div className="mb-6">
          <QuickDocumentUpload onDataExtracted={handleDocumentData} language={language} />
        </div>
